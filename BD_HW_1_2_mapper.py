...
class Mapper(object):
	...
	def emit(self, key, word, value=1):
	...
	def map(self):
		...
		for line in self:
			...
			paper_title = line.strip().split(':::')[-1]
			
			for punc in string.punctuation:
				paper_title = paper_title.replace(punc,'')
			
			wrd_list=paper_title.lower().strip().split()
			
			for auth in authors:
			    for word in wrd_list:
			        if word not in allStopWords and len(word)>1:
				    	self.emit(auth,word,wrd_list.count(word))
